...
def calculate_cloud_free_coverage(masks):
    # 初始化“或”结果掩膜，形状为 256x256，全部为 0
    combined_mask = np.zeros((256, 256), dtype=np.uint8)
    
    # 对每个掩膜进行 (1 - mask) 操作，然后进行按位“或”操作
    for mask in masks:
        inverted_mask = (1 - mask ).astype(np.uint8)  # 求反掩膜（1 表示无云，0 表示有云）
        combined_mask = np.bitwise_or(combined_mask, inverted_mask)  # 逐元素“或”

    # 计算无云覆盖率
    total_pixels = combined_mask.size  # 总像素数
    cloud_free_pixels = np.sum(combined_mask)  # 合并掩膜中值为1（无云）的像素数
    cloud_free_coverage = cloud_free_pixels / total_pixels  # 无云覆盖率
    
    return cloud_free_coverage

def calculate_average_errors(images, masks, cloud_free_image):
    errors = []
    cloud_free_image = cloud_free_image[0]

    for image, mask in zip(images, masks):
        # 遮蔽有云区域，仅保留无云区域（mask 中为 0 的部分）
        cloud_free_mask = 1 - mask  # 将掩膜反转，无云区域变为 1
        masked_image = image * cloud_free_mask
        masked_cloud_free_image = cloud_free_image * cloud_free_mask  # 对无云图像也仅保留无云区域

        # 计算无云区域的平均误差
        non_cloud_pixels = cloud_free_mask.sum()  # 无云像素的数量
        if non_cloud_pixels > 0:  # 确保存在无云区域
            error = torch.sum(torch.abs(masked_image - masked_cloud_free_image)) / non_cloud_pixels
        else:
            error = float('nan')  # 如果没有无云区域，返回 NaN 表示无效

        # 将误差添加到列表中
        errors.append(error)
    
    # 计算所有图像误差的平均值，忽略 NaN 值
    errors_tensor = torch.tensor(errors, dtype=torch.float32)
    average_error = torch.nanmean(errors_tensor)  # 计算平均误差，忽略 NaN

    return average_error.item()  # 返回标量值

import torch
import matplotlib.pyplot as plt
import os

def visualize_and_save_errors(images, masks, cloud_free_image, save_dir='./saved_images'):
    errors = []
    cloud_free_image = cloud_free_image[0]
    ...
    for i, (image, mask) in enumerate(zip(images, masks)):
        ...
        # 对无云图像进行相同的操作
        rgb_cloud_free_image = cloud_free_image[[3, 2, 1], ...]

        # 遮蔽有云区域，仅保留无云区域
        cloud_free_mask = 1 - mask  # 将掩膜反转，无云区域变为 1
        masked_rgb_image = rgb_image * cloud_free_mask
        masked_rgb_cloud_free_image = rgb_cloud_free_image * cloud_free_mask  # 对无云图像也仅保留无云区域

        # 计算无云区域的平均误差
        non_cloud_pixels = cloud_free_mask.sum()  # 无云像素的数量
        if non_cloud_pixels > 0:  # 确保存在无云区域
            error = torch.sum(torch.abs(masked_rgb_image - masked_rgb_cloud_free_image)) / non_cloud_pixels
        else:
            error = float('nan')  # 如果没有无云区域，返回 NaN 表示无效
        ...
        # 绘制各个步骤的结果
        axs[0].imshow(rgb_image.permute(1, 2, 0).cpu())  # 将通道维度调整为最后
        axs[0].set_title("Original RGB Image")
        axs[0].axis('off')

        axs[1].imshow(mask, cmap='gray')
        axs[1].set_title("Mask (Cloudy Regions)")
        axs[1].axis('off')

        axs[2].imshow(masked_rgb_image.permute(1, 2, 0).cpu())  # 将通道维度调整为最后
        axs[2].set_title("Masked RGB Image (Cloud-Free Regions)")
        axs[2].axis('off')

        axs[3].imshow(rgb_cloud_free_image.permute(1, 2, 0).cpu())  # 显示无云图像
        axs[3].set_title("Cloud-Free Image")
        axs[3].axis('off')

        # 误差图（使用 'viridis' 颜色映射）
        error_image = torch.abs(masked_rgb_image - masked_rgb_cloud_free_image).sum(dim=0)
        axs[4].imshow(error_image.cpu(), cmap='coolwarm')  # 选择 'viridis' 颜色映射
        axs[4].set_title(f"Error Map\nMean Error: {error:.4f}")
        axs[4].axis('off')

        # 保存图像
        save_path = os.path.join(save_dir, f'image_{i}_error.png')
        plt.tight_layout()
...
class sen12mscrts(Dataset):
    def __init__(self, root, split="all", region='all', cloud_masks=None, 
                 sample_type='cloudy_cloudfree', sampler='rf_sampler', n_input_samples=3, 
    ...
    def filtered_long_range_sampler(self, coverage, clear_tresh = 1e-3, seed=None):
        ...
        specific_idx= [pdx for pdx, perc in enumerate(coverage) if perc >= min_cov and perc <= self.max_cov]

        if len(specific_idx) < self.n_input_t:
            specific_idx= [pdx for pdx, perc in enumerate(coverage) if perc >= clear_tresh]
            if len(specific_idx) < self.n_input_t:
                specific_idx = range(30)
        ...
        remaining_indices = [i for i in specific_idx if i != cloudless_idx and i != closest_idx]
    ...
    def rf_sampler(self, idx, coverage, clear_tresh = 1e-3, seed=None):
        ...
        # mean_dist = sum(abs(item - cloudy_idx) for item in reference_idx) / len(reference_idx)
        mean_dist = 0

        inputs_idx += reference_idx
        mean_cov = sum([coverage[pdx] for pdx in inputs_idx]) / self.n_input_t

        return inputs_idx, cloudless_idx, mean_dist, mean_cov

    def rf_sampler_dis_ablation(self, idx, clear_tresh = 1e-3, dis=1, seed=None):
        if self.split != 'train':
            seed = 4068
        else:
            np.random.seed(seed)
        ...
        def generate_indices(cloudless_index, cloudy_index, dist, n):
            indices = set(range(30))
            indices.discard(cloudless_index)  # 去掉 cloudless_index
            ...
            while len(result) < n:
                candidates = [
                    ...
                    if abs(idx - cloudy_index) == current_dist
                ]
                
                for candidate in candidates:
                    if len(result) < n:
                        result.append(candidate)
                        indices.discard(candidate)

                current_dist += 1

            return result

        rf_idx = generate_indices(cloudless_idx, cloudy_idx, dis, self.n_input_t-1)
    ...
    def rf_sampler_dis_ablation1(self, idx, coverage, clear_tresh = 1e-3, dis=1, seed=None):
        if self.split != 'train':
            seed = 4068
        else:
            np.random.seed(seed)
        ...
        def generate_indices(cloudless_index, cloudy_index, dist, n):
            indices = set(range(30))
            indices.discard(cloudless_index)

            result = {
                idx for idx in indices
                if dist <= abs(idx - cloudy_index) <= dist + 1
            }

            if len(result) >= n:
                return True, list(result)[:n]
            else:
                return False, []

        flag, rf_idx = generate_indices(cloudless_idx, cloudy_idx, dis, self.n_input_t-1)

        inputs_idx = []
        inputs_idx.append(cloudy_idx)
        inputs_idx += rf_idx

        return inputs_idx, cloudless_idx, flag
    ...
    def __getitem__(self, pdx):  # get the time series of one patch
        ...
        if self.sampling=='random':
            ...
            inputs_idx, cloudless_idx, coverage_match = self.fixed_sampler(coverage)
        elif self.sampling=='filtered_long_range_sampler':
            inputs_idx, cloudless_idx, coverage_match = self.filtered_long_range_sampler(coverage)
        elif self.sampling=='dist_ablation':
            inputs_idx, cloudless_idx, flag = self.rf_sampler_dis_ablation1(pdx, coverage, dis=self.dist)
        else:
            inputs_idx, cloudless_idx, mean_dist, mean_cov = self.rf_sampler(pdx, coverage)
        
        if self.sampling=='dist_ablation' and flag == False:
            return {}
        ...
        if self.split == 'test':
            in_avg_cov = 1 - calculate_cloud_free_coverage(in_masks)

            # save_dir = f'img_gen/error_map/exp-1-dist13/img_{pdx}'
            # in_avg_error = visualize_and_save_errors(in_s2, in_masks, tg_s2, save_dir)
        ...
        if self.split == 'test':
            sample['statistics'] = {#'Mean error': in_avg_error,
                                    'Mean cov': in_avg_cov,
                                    }
        
        if self.sampling=='dist_ablation':
            sample['statistics']['dist'] = self.dist
...
if __name__ == "__main__":
    ...
    for results in dataloader:
        ...
        input_data = torch.cat([sar_data, cloudy_data], dim=1)
        print(input_data[:,:2,...]==sar_data)
